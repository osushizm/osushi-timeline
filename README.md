...
#### ラベルが自動で付かないとき
GitHubのIssue Formsは、リポジトリに**まだ存在しないラベル**を
自動では付与できない(既知の仕様)。tsundokuラベルが未作成のまま
Issueを作ると、ラベル無しのIssueになることがある。
そのため、タイトルの接頭辞 `[積読] `(テンプレートの初期値のまま)
でも判定するフォールバックを入れてあるので、ラベルが付かなくても
拾われる。念のため一度だけ Issues → Labels → New label で
`tsundoku` ラベルを作っておくと、以後は確実にラベルも付くようになる。

#### 5件を超えたら
本・URLそれぞれ最大5件までがトップページに表示され、
超えた分は「→ あと◯件を見る」リンクから `/tsundoku/` ページで
全件見られる。
